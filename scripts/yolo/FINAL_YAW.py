...
MODEL_PATH = r"C:\Users\een23013\Downloads\best (2).pt"
IMAGE_PATH = r"C:\Users\een23013\Downloads\position_validation\position_validation\image6005.png"
...
if ids is not None:
    ...
    for i in range(len(ids)):
        ...
        forward = R_gt[:,1]
        print("Detected ArUco ID:", ids[i][0])
        print("Aruco +X yaw:", np.degrees(np.arctan2(R_gt[:,0][0], R_gt[:,0][1])))
        print("Aruco +Y yaw:", np.degrees(np.arctan2(R_gt[:,1][0], R_gt[:,1][1])))
...
if (
    ...
    yaw_error = angle_error(yaw_yolo, yaw_aruco)

    yaw_yolo_flipped = wrap_angle(yaw_yolo + 180)
    yaw_error_flipped = angle_error(yaw_yolo_flipped, yaw_aruco)

    if yaw_error_flipped < yaw_error:
        print("Detected 180 degree flip. Correcting YOLO yaw.")
        yaw_yolo = yaw_yolo_flipped
        yaw_error = yaw_error_flipped
