...
IMAGE_PATH = r"C:\Users\egn23014\Downloads\bridge_dataset\bridge_dataset\image2398.png"
...
def preprocess_letterbox(image):
    ...
    planar = img_rgb.transpose(2, 0, 1).flatten()

    frame = dai.ImgFrame()
    frame.setWidth(INPUT_SIZE)
    frame.setHeight(INPUT_SIZE)
    frame.setType(dai.ImgFrame.Type.RGB888p)
    frame.setData(planar)
...
def main():
    ...
    input_frame, shown_image = preprocess_letterbox(image)
    ...
    with dai.Device(pipeline) as device:
        input_q = device.getInputQueue("input")
        nn_q = device.getOutputQueue("nn", maxSize=1, blocking=True)

        print("Sending image to OAK-D SR...")
        input_q.send(input_frame)

        print("Waiting for neural network output...")
        nn_data = nn_q.get()

        print("Output layer names:", nn_data.getAllLayerNames())

        outputs = {}

        for name in nn_data.getAllLayerNames():
            data = np.array(nn_data.getLayerFp16(name), dtype=np.float32)
            print(f"Layer: {name}, length: {len(data)}")
            outputs[name] = data
        ...
        cv2.imshow("YOLOv8 pose on OAK-D SR", result)
        cv2.waitKey(0)
